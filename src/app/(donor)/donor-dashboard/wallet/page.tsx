        ...
        return (
          ...
          >
            <div className={cn(
              ...
              isSelected ? "border-blue-600 bg-blue-600 text-white" : "border-zinc-300 bg-white group-hover:border-zinc-400"
            )}>
              {isSelected && <Check className="w-3 h-3" strokeWidth={3} />}
            </div>

            <div className="h-10 w-14 bg-white border border-zinc-200 rounded-md flex items-center justify-center shrink-0 shadow-sm">
              {method.type === 'card' ? (
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                ...
                {method.isDefault && <Badge variant="secondary" className="text-[9px] px-1.5 h-4 bg-zinc-100 border-zinc-200">Default</Badge>}
              </div>
  ...
  return (
    <div className="space-y-3 pt-4 border-t border-zinc-100 text-left">
      <div className="flex items-center gap-2 mb-1">
        ...
        <Label className="text-zinc-700 font-semibold">Billing Address</Label>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        <Input 
          ...
          className="md:col-span-2 bg-white border-zinc-200 h-11 shadow-sm" 
        />
        <Input 
          placeholder="City" 
          value={address.city} 
          onChange={(e) => handleChange('city', e.target.value)}
          className="bg-white border-zinc-200 h-11 shadow-sm"
        />
...
const CardForm = ({ formData, setFormData, isEditing }: any) => (
  <>
    <div className="space-y-2 text-left">
        <Label className="text-zinc-700 font-semibold">Card Number</Label>
        <div className="relative group">
            <CreditCard className="absolute left-4 top-3.5 h-5 w-5 text-zinc-400 group-focus-within:text-blue-600 transition-colors" />
            <Input 
                placeholder="0000 0000 0000 0000" 
                className={cn(
                    "pl-12 h-12 bg-white border-zinc-200 shadow-sm transition-all focus:ring-2 focus:ring-blue-100 focus:border-blue-300 font-mono text-lg",
    ...
    <div className="grid grid-cols-2 gap-6 text-left">
        <div className="space-y-2">
            <Label className="text-zinc-700 font-semibold">Expiration</Label>
            <div className="relative">
                <Calendar className="absolute left-3 top-3.5 h-4 w-4 text-zinc-400" />
                <Input 
                    placeholder="MM/YY" 
                    className="h-12 pl-9 bg-white border-zinc-200 shadow-sm text-center font-mono text-lg"
        ...
        <div className="space-y-2">
            <Label className="text-zinc-700 font-semibold">CVC</Label>
            <Input 
                placeholder="123" 
                className="h-12 bg-white border-zinc-200 shadow-sm text-center font-mono text-lg"
                value={formData.cvc} 
    ...
    <div className="space-y-2 text-left">
        <Label className="text-zinc-700 font-semibold">Cardholder Name</Label>
        <div className="relative">
            <User className="absolute left-4 top-3.5 h-5 w-5 text-zinc-400" />
            <Input 
                placeholder="JOHN DOE" 
                className="h-12 pl-12 bg-white border-zinc-200 shadow-sm font-medium uppercase placeholder:normal-case"
                value={formData.name} 
    ...
    <div className="flex items-center justify-center gap-2 text-xs text-zinc-500 bg-white py-3 rounded-xl border border-zinc-200 mt-4 shadow-sm">
...
const BankForm = ({ formData, setFormData, isEditing }: any) => (
  <>
    <div className="bg-emerald-50 text-emerald-800 text-sm p-4 rounded-xl flex items-start gap-3 border border-emerald-100 shadow-sm text-left">
    ...
    <div className="space-y-5 text-left">
        <div className="space-y-2">
            <Label htmlFor="routing" className="text-zinc-700 font-semibold">Routing Number</Label>
            <div className="relative group">
                <Landmark className="absolute left-4 top-3.5 h-5 w-5 text-zinc-400 group-focus-within:text-zinc-600 transition-colors" />
                <Input 
                    id="routing"
                    placeholder="9 Digit Routing Number" 
                    className={cn(
                        "pl-12 h-12 bg-white border-zinc-200 shadow-sm font-mono text-lg transition-all",
                        isEditing && "bg-zinc-100 text-zinc-500 border-zinc-100 cursor-not-allowed"
                    )}
        ...
        <div className="space-y-2">
            <Label htmlFor="account" className="text-zinc-700 font-semibold">Account Number</Label>
            <div className="relative group">
                <Building2 className="absolute left-4 top-3.5 h-5 w-5 text-zinc-400 group-focus-within:text-zinc-600 transition-colors" />
                <Input 
                    id="account"
        ...
        <div className="space-y-2">
            <Label htmlFor="holder" className="text-zinc-700 font-semibold">Account Holder Name</Label>
            <div className="relative group">
                <User className="absolute left-4 top-3.5 h-5 w-5 text-zinc-400" />
                <Input 
                    id="holder"
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-24">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 px-1 text-left">
        <div>
          <h1 className="text-3xl md:text-4xl font-bold text-zinc-900 tracking-tight">Wallet</h1>
          <p className="text-zinc-500 mt-2 text-lg">Manage your payment methods securely.</p>
        </div>
        <Button onClick={openAddModal} className="bg-zinc-900 hover:bg-zinc-800 text-white shadow-xl hover:shadow-2xl hover:-translate-y-0.5 transition-all h-12 px-6 font-bold rounded-full">
      ...
      <AnimatePresence>
        {showACHNudge && (
          <motion.div 
            ...
            className="bg-gradient-to-r from-emerald-50 to-teal-50 border border-emerald-100 rounded-3xl p-6 relative overflow-hidden shadow-sm group text-left"
          >
      ...
      <div className="space-y-8">
         {methods.length === 0 && (
            <div className="text-center py-20 bg-zinc-50 rounded-3xl border-2 border-dashed border-zinc-200">
               ...
               <h3 className="text-xl font-bold text-zinc-900 mb-2">No payment methods yet</h3>
               <p className="text-zinc-500 mb-6">Add a card or bank account to start giving.</p>
            ...
            {methods.map((method, idx) => {
               ...
               return (
                  <motion.div 
                     ...
                     key={method.id}
                     className="group bg-white rounded-[2rem] border border-zinc-200 p-2 shadow-sm hover:shadow-xl transition-all duration-500 overflow-hidden text-left"
                  >
                     <div className="flex flex-col lg:flex-row gap-8 p-6 lg:p-8">
                        ...
                        <div className="flex-1 flex flex-col min-w-0">
                           <div className="flex items-start justify-between mb-6">
                              <div>
                                 <div className="flex items-center gap-3 mb-1.5">
                                    <h3 className="text-2xl font-bold text-zinc-900 capitalize tracking-tight">
                              ...
                              <DropdownMenu>
                                 <DropdownMenuTrigger asChild>
                                    <Button variant="ghost" size="icon" className="h-10 w-10 text-zinc-400 hover:text-zinc-900 hover:bg-zinc-100 rounded-full">
                                       <MoreHorizontal className="h-5 w-5" />
                                    </Button>
                                 </DropdownMenuTrigger>
                                 <DropdownMenuContent align="end" className="w-56 p-2 rounded-xl">
                                    <DropdownMenuLabel className="text-xs text-zinc-400 uppercase tracking-wider font-bold px-2 py-1.5">Manage Method</DropdownMenuLabel>
                           ...
                           <div className="mb-6 flex gap-3 items-start">
                              <div className="p-1.5 bg-zinc-50 rounded-md text-zinc-400 mt-0.5">
                                <MapPin className="h-4 w-4" />
                              </div>
                              <div>
                                <p className="text-xs font-bold text-zinc-400 uppercase tracking-wider mb-0.5">Billing Address</p>
                           ...
                           <div className="mt-auto pt-6 border-t border-zinc-100">
                              <p className="text-xs font-bold text-zinc-400 uppercase tracking-widest mb-4 flex items-center gap-2">
                                 Connected Impact <span className="bg-zinc-100 text-zinc-600 px-1.5 py-0.5 rounded text-[10px] min-w-[20px] text-center">{attachedPledges.length}</span>
                              </p>
                              ...
                              {attachedPledges.length > 0 ? (
                                 <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    {attachedPledges.map(pledge => (
                                       <div key={pledge.id} className="flex items-center gap-4 p-3 rounded-2xl bg-zinc-50/80 border border-zinc-100 hover:border-zinc-300 hover:bg-white transition-all group/pledge cursor-default shadow-sm hover:shadow-md">
                                          ...
                                          <div className="flex-1 min-w-0">
                                             <p className="text-sm font-bold text-zinc-900 truncate">{pledge.name}</p>
                                             <p className="text-xs text-zinc-500 font-medium">{formatCurrency(pledge.amount)} / {pledge.frequency}</p>
                                          </div>
                                 ...
                                 <div className="flex items-center gap-3 text-sm text-zinc-500 italic bg-zinc-50/50 p-4 rounded-xl border border-dashed border-zinc-200">
                                    <div className="p-2 bg-white rounded-full shadow-sm"><Wallet className="h-4 w-4 text-zinc-400" /></div>
      ...
      <Dialog open={isMethodModalOpen} onOpenChange={setIsMethodModalOpen}>
         <DialogContent className="sm:max-w-[600px] p-0 overflow-hidden gap-0 rounded-3xl max-h-[90vh] overflow-y-auto">
            <DialogHeader className="p-8 pb-4 bg-white border-b border-zinc-100 sticky top-0 z-10 text-left">
               ...
               {!editingMethod ? (
                  <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as any)} className="w-full">
                     <TabsList className="grid w-full grid-cols-2 mb-8 bg-white p-1 rounded-xl shadow-sm border border-zinc-200 h-12">
                        <TabsTrigger value="card" className="rounded-lg font-bold data-[state=active]:bg-zinc-900 data-[state=active]:text-white transition-all">Credit Card</TabsTrigger>
                        <TabsTrigger value="bank" className="rounded-lg font-bold data-[state=active]:bg-zinc-900 data-[state=active]:text-white transition-all">Bank Account</TabsTrigger>
                     </TabsList>
            ...
            <DialogFooter className="p-6 bg-white border-t border-zinc-100 flex flex-col sm:flex-row gap-3 sticky bottom-0 z-10">
               ...
               <Button onClick={handleSaveMethod} className="bg-zinc-900 hover:bg-zinc-800 text-white shadow-lg h-12 px-8 font-bold rounded-xl transition-transform active:scale-95">
      ...
      <Dialog open={isSwapPledgeOpen} onOpenChange={setIsSwapPledgeOpen}>
         <DialogContent className="sm:max-w-[500px] border-none shadow-2xl rounded-3xl overflow-hidden p-0 gap-0">
            <DialogHeader className="p-8 pb-6 bg-zinc-50 border-b border-zinc-100 text-center">
                ...
                <DialogTitle className="text-xl font-bold text-zinc-900">Move Support</DialogTitle>
            ...
            <div className="p-8 space-y-8 bg-white min-h-[300px] max-h-[60vh] overflow-y-auto text-left">
                <div className="flex flex-col items-center gap-4">
                    <div className="bg-white p-4 rounded-2xl border border-zinc-200 shadow-sm flex items-center gap-4 w-full">
                        ...
                        <div className="flex-1">
                            <p className="font-bold text-zinc-900">{pledgeToSwap?.name}</p>
                            <div className="flex items-center gap-2 mt-0.5">
                                <Badge variant="secondary" className="px-1.5 py-0 text-[10px] font-bold uppercase tracking-wider">{pledgeToSwap?.frequency}</Badge>
                                  <span className="text-sm font-medium text-zinc-600">{formatCurrency(pledgeToSwap?.amount || 0)}</span>
                            </div>
                        </div>
                    </div>
                ...
                <div className="space-y-4">
                    <Label className="text-sm font-bold text-zinc-900 uppercase tracking-wider ml-1">Move To</Label>
                    {methods.filter(m => m.id !== pledgeToSwap?.paymentMethodId).length > 0 ? (
                        ...
                        <div className="p-6 bg-zinc-50 rounded-xl border border-dashed border-zinc-200 text-center space-y-3">
                            <div className="w-10 h-10 bg-white rounded-full flex items-center justify-center mx-auto shadow-sm">
                                <Wallet className="h-5 w-5 text-zinc-400" />
                            </div>
                            <div>
                                <p className="text-sm font-bold text-zinc-900">No other payment methods</p>
                                <p className="text-xs text-zinc-500 mt-1">Add a new card or bank account to move this support.</p>
                            </div>
      ...
      <Dialog open={isMovePledgesOpen} onOpenChange={setIsMovePledgesOpen}>
         <DialogContent className="sm:max-w-[500px] border-none shadow-2xl rounded-3xl overflow-hidden p-0 gap-0">
            <DialogHeader className="p-8 pb-6 bg-amber-50 border-b border-amber-100 text-center">
               ...
               <DialogTitle className="text-2xl font-bold text-amber-950">Active Support Detected</DialogTitle>
               <DialogDescription className="pt-2 text-base text-amber-800/80 leading-relaxed">
                  You are removing a payment method that funds <strong>{pledges.filter(p => p.paymentMethodId === methodToDelete).length} active ministries</strong>.
            ...
            <div className="p-8 space-y-8 bg-white min-h-[300px] max-h-[60vh] overflow-y-auto text-left">
               <div className="space-y-3">
                  <p className="text-xs font-bold text-zinc-400 uppercase tracking-widest ml-1">Support to Transfer</p>
                  <ul className="space-y-2">
                     {pledges.filter(p => p.paymentMethodId === methodToDelete).map(p => (
                        <li key={p.id} className="text-sm flex items-center justify-between bg-zinc-50 p-3 rounded-xl border border-zinc-100">
                           ...
                           <span className="font-mono font-medium text-zinc-600 text-xs">{formatCurrency(p.amount)}</span>
                        </li>
                     ))}
                  </ul>
               </div>
               ...
               <div className="space-y-4">
                  <Label className="text-zinc-700 font-bold uppercase tracking-widest text-xs ml-1">Move All To</Label>
            ...
            <DialogFooter className="p-6 bg-zinc-50 border-t border-zinc-100 flex justify-between">
               <Button variant="ghost" onClick={() => setIsMovePledgesOpen(false)} className="text-zinc-500 hover:text-zinc-900">Cancel</Button>
               <Button 
                  onClick={executeMoveAndDelete} 
                  disabled={!targetMethodId}
                  className="bg-zinc-900 hover:bg-zinc-800 text-white shadow-lg h-11 px-6 font-bold rounded-xl"
               >
                  Transfer & Delete
               </Button>
            </DialogFooter>
