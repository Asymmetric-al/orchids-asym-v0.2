...
import { 
  ...
  ImageOff, Wand2, Link as LinkIcon,
          ...
          <button
            ...
            className={cn(
              ...
              current === type
                ...
                : "bg-white text-zinc-600 border-zinc-200/60 hover:bg-zinc-50 hover:border-zinc-300 hover:text-zinc-900 shadow-sm"
            )}
          >
...
const PostActions = ({ 
  ...
  const shareUrl = `https://orchids.app/posts/${post.id}`; // Mock URL
  const shareText = `Check out this update from ${post.workerName} on Orchids!`;
  ...
  const handleNativeShare = () => {
    if (navigator.share) {
      navigator.share({
        title: post.title || 'Update from Orchids',
        text: shareText,
        url: shareUrl,
      }).catch(console.error);
    }
    ...
    <div className="flex items-center justify-between py-2 mt-8">
      <div className="flex items-center gap-2">
        ...
        <motion.button 
          ...
          className={cn(
            "flex items-center gap-2 px-3 py-2 rounded-full transition-all duration-200 group hover:bg-zinc-50",
            post.prayed ? "text-indigo-600" : "text-zinc-500 hover:text-zinc-900"
          )}
        >
      ...
      <div className="flex items-center gap-1">
        <motion.button 
          ...
          className={cn(
            ...
            post.saved ? "text-blue-600 bg-blue-50" : "text-zinc-400 hover:text-zinc-900 hover:bg-zinc-100"
          )}
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-56">
            ...
            <DropdownMenuItem onClick={() => window.open(`mailto:?subject=${encodeURIComponent(post.title || 'Update from Orchids')}&body=${encodeURIComponent(shareText + '\n\n' + shareUrl)}`)}>
...
const CommentsSection = ({ 
  ...
  const handleAIGenerate = async (tone: 'Encouraging' | 'Prayer' | 'Celebratory') => {
    ...
    // Mocking AI response as GoogleGenAI is not in package.json
    setTimeout(() => {
  ...
  return (
    ...
    >
      <h4 className="text-xs font-bold text-zinc-400 uppercase tracking-wider mb-6">Discussion ({comments.length})</h4>
            ...
            <div className="flex gap-4">
                ...
                <div className="flex-1 space-y-1 text-left">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                        <span className="text-sm font-bold text-zinc-900">{comment.author}</span>
                        {comment.authorTitle && (
                            <Badge variant="secondary" className="text-[10px] h-4 px-1.5 bg-zinc-200/50 text-zinc-600 font-medium">
                            {comment.authorTitle}
                            </Badge>
                        )}
                        </div>
                        <span className="text-xs text-zinc-400 font-medium">{comment.time}</span>
                    </div>
                    <p className="text-sm text-zinc-700 leading-relaxed font-normal">{comment.text}</p>
                    <button 
                    ...
                    {comment.replies.map(reply => (
                        <div key={reply.id} className="flex gap-3 text-left">
                            ...
                            <div className="flex-1 space-y-1">
                                <div className="flex items-center gap-2">
                                    <span className="text-sm font-bold text-zinc-900">{reply.author}</span>
                                    {reply.authorTitle && (
                                ...
                                <p className="text-sm text-zinc-700 leading-relaxed font-normal">{reply.text}</p>
                            </div>
                        </div>
                    ))}
                </div>
                ...
                <div className="ml-12 mt-3 pl-3">
                    <div className="relative group">
                        <Input 
                            ...
                            className="pr-10 bg-white border-zinc-200 h-9 text-xs shadow-sm pl-3"
                            value={replyText}
                        ...
                        <button 
                            ...
                            className="absolute right-1 top-1 p-1 text-zinc-400 hover:text-blue-600 transition-colors"
                        >
      ...
      <div className="flex gap-2 mb-3 overflow-x-auto no-scrollbar">
        <button 
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-white border border-zinc-200 text-xs font-medium text-zinc-600 hover:bg-zinc-50 hover:border-zinc-300 transition-colors whitespace-nowrap shadow-sm"
        >
      ...
      <div className="flex gap-3 items-center">
        ...
        <div className="relative flex-1 group">
          <Input 
            ...
            className="pr-12 bg-white border-zinc-200/80 focus:border-zinc-300 focus:ring-4 focus:ring-zinc-100 rounded-full h-12 transition-all shadow-sm pl-5"
            value={text}
          ...
          <button 
            ...
            disabled={!text.trim()}
            className="absolute right-1.5 top-1.5 p-2 text-white bg-zinc-900 hover:bg-zinc-800 rounded-full disabled:opacity-0 disabled:scale-90 transition-all shadow-sm w-9 h-9 flex items-center justify-center"
          >
...
const PostCard: React.FC<{ 
  ...
  return (
    <motion.article 
      ...
      className="bg-white rounded-3xl border border-zinc-200 shadow-sm p-6 sm:p-8 overflow-hidden hover:shadow-md transition-shadow duration-300"
    >
      {/* Meta Header */}
        ...
        <div className="flex items-center gap-3">
          <div className="relative group cursor-pointer">
            <Avatar className="h-10 w-10 border border-zinc-100 shadow-sm transition-transform group-hover:scale-105">
              ...
              <AvatarFallback className="bg-zinc-100 font-bold text-zinc-600">{post.workerName[0]}</AvatarFallback>
            </Avatar>
          </div>
          <div className="text-left">
            <div className="flex items-center gap-2">
              <h3 className="font-bold text-zinc-900 text-sm leading-none cursor-pointer hover:underline decoration-2 decoration-zinc-200 underline-offset-4">{post.workerName}</h3>
            ...
            <div className="flex items-center gap-1.5 text-xs text-zinc-500 mt-1 font-medium">
              <span className="text-zinc-400">{post.readTime || '3 min read'}</span>
      ...
      <div className="space-y-6 text-left">
        {post.title && (
          <h2 className="text-3xl font-extrabold text-zinc-900 tracking-tight leading-tight">{post.title}</h2>
        )}
        ...
        {imageError && (
           <div className="rounded-2xl overflow-hidden border border-zinc-100 bg-zinc-50 h-32 flex items-center justify-center text-zinc-400">
        ...
        <div 
          className="prose prose-zinc prose-lg max-w-none prose-headings:font-bold prose-headings:tracking-tight prose-p:leading-relaxed prose-p:text-zinc-600 prose-blockquote:border-l-4 prose-blockquote:border-zinc-900 prose-blockquote:pl-6 prose-blockquote:italic prose-blockquote:font-medium prose-blockquote:text-zinc-800 prose-a:text-blue-600 prose-a:no-underline hover:prose-a:underline prose-img:rounded-xl"
  ...
  return (
    <div className="max-w-2xl mx-auto pb-20">
      ...
      <div className="px-1 mb-8 pt-8 text-center sm:text-left">
        <h1 className="text-4xl md:text-6xl font-bold tracking-tighter text-zinc-900 mb-4">
          Your Feed
        </h1>
      ...
      <div className="space-y-6">
        ...
        {filteredPosts.length === 0 && (
          <div className="py-32 text-center">
            <div className="inline-flex items-center justify-center w-20 h-20 rounded-full bg-zinc-50 text-zinc-300 mb-6">
              {filter === 'Saved' ? (
      ...
      {filteredPosts.length > 0 && (
        <div className="flex flex-col items-center py-20 space-y-4 opacity-40 hover:opacity-100 transition-opacity">
          ...
          <p className="text-xs font-bold text-zinc-500 uppercase tracking-widest pt-2">You&apos;re all caught up</p>
        </div>
      )}
    </div>
  );
