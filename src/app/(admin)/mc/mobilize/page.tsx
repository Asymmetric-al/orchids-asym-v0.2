...
const StatCard = ({ title, value, icon: Icon, color }: { title: string, value: number, icon: any, color: string }) => (
    <Card className="border-slate-200 shadow-sm rounded-xl">
      ...
      <div className={cn("h-10 w-10 rounded-lg flex items-center justify-center bg-opacity-10", color.replace('text-', 'bg-'))}>
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-20">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 pb-6 border-b border-zinc-100">
        <div className="text-left">
          <h1 className="text-3xl font-black tracking-tighter text-zinc-900 lg:text-4xl">Mobilization</h1>
          <p className="text-zinc-500 mt-1 text-xs font-bold uppercase tracking-wider">Manage missionary pipeline, vetting, and deployment.</p>
        </div>
        <Button className="bg-zinc-900 text-white shadow-md hover:bg-zinc-800 rounded-lg px-6 h-9 text-[10px] font-bold uppercase tracking-wider" onClick={() => setIsAddSheetOpen(true)}>
        ...
        <div className="p-3 border-b border-slate-100 flex flex-col lg:flex-row justify-between items-center gap-3 bg-slate-50/50">
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full lg:w-auto">
            <TabsList className="bg-white border border-slate-200 h-8 rounded-lg p-0.5">
              <TabsTrigger value="all" className="h-7 text-[10px] uppercase font-bold px-3 rounded-md">All</TabsTrigger>
              <TabsTrigger value="applied" className="h-7 text-[10px] uppercase font-bold px-3 rounded-md">Applied</TabsTrigger>
              <TabsTrigger value="vetting" className="h-7 text-[10px] uppercase font-bold px-3 rounded-md">Vetting</TabsTrigger>
              <TabsTrigger value="training" className="h-7 text-[10px] uppercase font-bold px-3 rounded-md">Training</TabsTrigger>
              <TabsTrigger value="ready" className="h-7 text-[10px] uppercase font-bold px-3 rounded-md">Ready</TabsTrigger>
            </TabsList>
          </Tabs>
          ...
          <div className="flex items-center gap-2 w-full lg:w-auto">
            ...
            <Button variant="outline" size="icon" className="bg-white border-slate-200 shadow-none h-8 w-8 rounded-lg">
          ...
          <Table>
            <TableHeader className="bg-slate-50">
              <TableRow className="h-10 hover:bg-transparent">
                <TableHead className="text-[10px] font-bold uppercase tracking-wider h-10 px-4">Candidate</TableHead>
                <TableHead className="text-[10px] font-bold uppercase tracking-wider h-10 px-4">Role</TableHead>
                <TableHead className="text-[10px] font-bold uppercase tracking-wider h-10 px-4">Stage</TableHead>
                <TableHead className="text-[10px] font-bold uppercase tracking-wider h-10 px-4">Readiness</TableHead>
                <TableHead className="text-right text-[10px] font-bold uppercase tracking-wider h-10 px-4">Actions</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {filteredCandidates.map((candidate) => (
                <TableRow key={candidate.id} className="hover:bg-slate-50/50 cursor-pointer h-12" onClick={() => setSelectedCandidate(candidate)}>
                  <TableCell className="px-4 py-2">
                    <div className="flex items-center gap-2.5">
                      <Avatar className="h-8 w-8 bg-slate-100 border border-slate-200 rounded-lg">
                        <AvatarFallback className="text-[10px] font-bold text-slate-600">{candidate.name.substring(0, 2).toUpperCase()}</AvatarFallback>
                      </Avatar>
                      <div className="text-left">
                        <div className="text-xs font-bold text-slate-900 leading-tight">{candidate.name}</div>
                        <div className="text-[10px] text-slate-500 leading-tight">{candidate.email}</div>
                      </div>
                    </div>
                  </TableCell>
                  <TableCell className="px-4 py-2">
                    <div className="flex flex-col text-left">
                      <span className="text-xs font-semibold text-slate-700 leading-tight">{candidate.role}</span>
                  ...
                  <TableCell className="px-4 py-2">
                    <Badge variant="outline" className={cn("text-[10px] font-bold border shadow-none px-1.5 py-0 rounded-md uppercase tracking-wider", STAGE_COLORS[candidate.stage])}>
                      {candidate.stage}
                    </Badge>
                  </TableCell>
                  <TableCell className="px-4 py-2">
                    <div className="w-24 space-y-1">
                      ...
                      <Progress value={candidate.readiness} className="h-1 rounded-full" />
                    </div>
                  </TableCell>
                  <TableCell className="text-right px-4 py-2">
                    <Button variant="ghost" size="icon" className="h-8 w-8 rounded-lg" onClick={(e) => { e.stopPropagation(); setSelectedCandidate(candidate); }}>
                  ...
                  <div className="flex gap-5">
                    ...
                    <div className="text-left">
                      ...
                      <div className="flex gap-2 mt-3">
                        <Badge variant="secondary" className={cn("border shadow-none", STAGE_COLORS[selectedCandidate.stage])}>
                          {selectedCandidate.stage}
                        </Badge>
                        {selectedCandidate.tags.map(tag => (
                          <Badge key={tag} variant="outline" className="bg-white text-slate-600 border-slate-200 shadow-none">
                            {tag}
                          </Badge>
                        ))}
                      </div>
